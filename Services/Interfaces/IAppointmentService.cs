...
namespace HospitalManagementSystem.Services.Interfaces
{
    ...
    Task<List<Appointment>> GetAppointmentsByTypeAsync(string type);

        // Statistics
        Task<int> GetTotalAppointmentsCountAsync();
    Task<int> GetAppointmentsCountByStatusAsync(string status);
        ...
        // Soft delete & audit
        Task<bool> SoftDeleteAppointmentAsync(string id, string deletedBy);
        Task<List<Appointment>> GetDeletedAppointmentsAsync();
        Task<bool> RestoreAppointmentAsync(string id);

        // Collection operations
        Task<bool> ClearAllAppointmentsAsync();
    }
}
